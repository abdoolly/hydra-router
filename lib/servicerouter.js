...
class ServiceRouter {
  ...
  init(config, routesObj) {
    ...
    Object.keys(routesObj).forEach((serviceName) => {
      ...
      routes.forEach((routePattern) => {
        ...
        // is an external route pattern
        if (typeof routePattern === 'object' && routePattern.route) {
          isExternal = true;
          // jsut set the actual route after identifying that it's an external route
      ...
      for (let routeEntry of this.routerTable[serviceName]) {
        ...
        if (matchTest) {
          return {
            ...
            isExternal: routeEntry.isExternal
          };
        }
      }
    }
          ...
          if (request.headers['content-encoding'] && request.headers['content-encoding'].includes('gzip')) {
            zlib.gunzip(newBody, (err, unzipped) => {
              if (err) {
                ...
                this._processHTTPRequest(tracer, newerBody, matchResult.serviceName, requestUrl, request, response, resolve, matchResult.isExternal);
              }
            });
          } else {
            this._processHTTPRequest(tracer, newBody, matchResult.serviceName, requestUrl, request, response, resolve, matchResult.isExternal);
          }
        });
      } else {
        this._processHTTPRequest(tracer, null, matchResult.serviceName, requestUrl, request, response, resolve, matchResult.isExternal);
      }
    });
  }
  ...
  * @return {undefined}
  */
  async _processHTTPRequest(tracer, body, serviceName, requestUrl, request, response, resolve, isExternal = false) {
    ...
    try {
      let data = await hydra.makeAPIRequest(msg, { timeout: this.requestTimeout, isExternal });
